  ...
  methods: {
    ...
    async loadSports() {
      try {
        ...
        console.log('✅ Sports chargés directement dans SportField:', sportsData.length);
      } catch (error) {
        console.error('❌ Erreur lors du chargement des sports dans SportField:', error);
      } finally {
    ...
    onSearchSports(event) {
      ...
      console.log('🔍 Recherche sports avec query:', query, 'Résultats:', this.sportSearchResults.length);
    },
    onSportSelect(event) {
    ...
    async onDropdownClick() {
      console.log('🔽 Clic sur le bouton dropdown pour événement', this.eventIndex);
    ...
    onDropdownShow() {
      console.log('📋 Menu déroulant des sports affiché pour événement', this.eventIndex);
    ...
    async onInputFocus() {
      console.log('🎯 Focus sur le champ de saisie sport pour événement', this.eventIndex);
