...
const showDeleteBankrollDialog = ref(false);
  ...
  try {
    ...
    if (data.success) {
      ...
      toast.add({
        severity: "error",
        summary: "Erreur",
        detail: data.message || "Échec de la mise à jour des paramètres",
        life: 3000,
      });
    }
  } catch (error) {
    toast.add({
      severity: "error",
      summary: "Erreur",
      detail: error.message || "Erreur lors de la mise à jour des paramètres",
      life: 3000,
    });
...
function hideMenuOnClickOutside(e) {
  if (
    !menu.value.$el.contains(e.target) &&
    e.target !== document.getElementById("avatarImg")
  ) {
    menu.value.hide();
  ...
  try {
    ...
    if (data.success) {
      ...
      toast.add({
        severity: "error",
        summary: "Erreur",
        detail: data.message || "Échec de la suppression de l'avatar",
        life: 3000,
      });
    }
  } catch (error) {
    toast.add({
      severity: "error",
      summary: "Erreur",
      detail: error.message || "Erreur lors de la suppression de l'avatar",
      life: 3000,
    });
...
async function onAvatarSelected(event) {
  ...
  try {
    toast.add({
      severity: "info",
      summary: "En cours",
      detail: "Téléchargement de l'avatar en cours...",
      life: 2000,
    });
    ...
    if (data.success) {
      ...
      toast.add({
        severity: "error",
        summary: "Erreur",
        detail: data.message || "Échec de l'upload d'avatar",
        life: 3000,
      });
    }
  } catch (error) {
    ...
    toast.add({
      severity: "error",
      summary: "Erreur",
      detail: error.message || "Erreur lors du téléchargement de l'avatar",
      life: 3000,
    });
...
async function openBankrollDialog(bankroll = null) {
  if (bankroll) {
    ...
    try {
      const bankrollBookmakers = await BankrollService.getBankrollBookmakers(
        bankroll.id
      );
      ...
      existingBookmakers = bankrollBookmakers.map((userBookmaker) => {
        ...
        const availableBookmaker = availableBookmakers.value.find(
          (bm) => bm.id === userBookmaker.bookmakers_id
        );
        return {
      ...
      console.log(
        "Bookmakers formatés pour le formulaire:",
        existingBookmakers
      );
    } catch (error) {
      console.error(
        "Erreur lors du chargement des bookmakers existants:",
        error
      );
      toast.add({
        severity: "error",
...
async function saveBankroll() {
  if (!newBankroll.value.name.trim()) {
    toast.add({
      severity: "error",
      summary: "Erreur",
      detail: "Le nom de la bankroll est requis",
      life: 3000,
    });
    return;
  }
  ...
  const totalInitialAmount = newBankroll.value.bookmakers.reduce(
    (total, bm) => {
  ...
  const hasNewBookmakers = newBankroll.value.bookmakers.some(
    (bm) => !bm.userBookmakerId && bm.bookmaker && bm.initialAmount > 0
  );
  ...
  try {
    ...
    if (editingBankroll.value) {
      ...
      console.log(
        "Modification - Bookmakers à traiter:",
        newBankroll.value.bookmakers
      );
      ...
      const bookmarkersToDelete = originalBookmakers.value.filter(
        (original) => !currentBookmakerIds.includes(original.id)
      );
      ...
      for (const bookmakerToDelete of bookmarkersToDelete) {
        try {
          ...
          console.error(
            "Erreur lors de la suppression du bookmaker:",
            bookmakerToDelete.id,
            deleteError
          );
      ...
      for (const bookmaker of newBankroll.value.bookmakers) {
        ...
        if (bookmaker.bookmaker && bookmaker.initialAmount > 0) {
          ...
          const selectedBookmaker = availableBookmakers.value.find(
            (bm) => bm.slug === bookmaker.bookmaker
          );
          console.log(
            "Modification - Bookmaker sélectionné pour le slug",
            bookmaker.bookmaker,
            ":",
            selectedBookmaker
          );
          if (selectedBookmaker) {
            ...
            try {
              if (bookmaker.userBookmakerId) {
                ...
                await BankrollService.updateUserBookmaker(
                  bookmaker.userBookmakerId,
                  userBookmakerData
                );
                console.log(
                  "Modification - Association bookmaker mise à jour avec succès:",
                ...
                console.log(
                  "Modification - Nouvelle association bookmaker créée avec succès:",
                  selectedBookmaker.name
                );
              }
            } catch (bookmakerError) {
              console.error(
                "Modification - Erreur lors du traitement du bookmaker:",
                selectedBookmaker.name,
                bookmakerError
              );
              bookmakerErrors.push(
                `${selectedBookmaker.name}: ${bookmakerError.message}`
              );
            }
          } else {
            console.error(
              "Modification - Bookmaker non trouvé pour le slug:",
              bookmaker.bookmaker
            );
            bookmakerErrors.push(
              `Bookmaker non trouvé: ${bookmaker.bookmaker}`
            );
          }
        }
      }
      ...
      for (const bookmaker of newBankroll.value.bookmakers) {
        ...
        if (bookmaker.bookmaker && bookmaker.initialAmount > 0) {
          ...
          console.log(
            "Bookmaker sélectionné pour le slug",
            bookmaker.bookmaker,
            ":",
            selectedBookmaker
          );
          if (selectedBookmaker) {
            ...
            console.log(
              "Données à envoyer pour l'association:",
              userBookmakerData
            );
            ...
            try {
              ...
              console.log(
                "Association bookmaker créée avec succès:",
                selectedBookmaker.name
              );
            } catch (bookmakerError) {
              console.error(
                "Erreur lors de l'association du bookmaker:",
                selectedBookmaker.name,
                bookmakerError
              );
              bookmakerErrors.push(
            ...
            console.error(
              "Bookmaker non trouvé pour le slug:",
              bookmaker.bookmaker
            );
            bookmakerErrors.push(
              `Bookmaker non trouvé: ${bookmaker.bookmaker}`
...
function deleteBankroll(bankroll) {
  bankrollToDelete.value = bankroll;
  confirmationName.value = "";
  showDeleteBankrollDialog.value = true;
}

function closeDeleteBankrollDialog() {
  showDeleteBankrollDialog.value = false;
  bankrollToDelete.value = null;
  confirmationName.value = "";
  isDeleting.value = false;
}

async function confirmDeleteBankroll() {
  if (!bankrollToDelete.value) return;

  if (confirmationName.value !== bankrollToDelete.value.bankroll_name) {
    toast.add({
      severity: "warn",
      summary: "Attention",
      detail: "Le nom saisi ne correspond pas au nom de la bankroll",
      life: 3000,
    });
    return;
  }
  ...
  try {
    await BankrollService.deleteBankroll(
      bankrollToDelete.value.id,
      bankrollToDelete.value.bankroll_name
    );

    toast.add({
      severity: "success",
      summary: "Succès",
      detail: "Bankroll et toutes ses données associées supprimées avec succès",
      life: 3000,
    });

    closeDeleteBankrollDialog();
    await loadBankrolls();
  } catch (err) {
...
<template>
    ...
    <div class="flex flex-col gap-8">
      <div class="card flex flex-col items-center gap-4 w-full p-6">
        ...
        <img
          ...
          :src="getAvatarUrl()"
          alt="Avatar"
          class="w-24 h-24 rounded-full border-2 border-gray-300 cursor-pointer"
          id="avatarImg"
      ...
      <div class="card flex flex-col gap-4 w-full">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Button
            ...
            :key="option.slug"
            @click="selectedOption = option.slug"
            :class="{
              'bg-blue-500 text-white': selectedOption === option.slug,
            }"
        ...
        <template v-else>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select
                v-model="selectedDisplayBetView"
                :options="displayBetViewOptions"
                optionLabel="name"
                optionValue="slug"
                class="w-full"
              ></Select>
            ...
            <div>
              ...
              <Select
                v-model="selectedDisplayDashboard"
                :options="displayDashboardOptions"
                optionLabel="name"
                optionValue="slug"
                class="w-full"
              ></Select>
            ...
            <div>
              ...
              <Select
                v-model="selectedDuplicateBetDate"
                :options="duplicateBetDateOptions"
                optionLabel="name"
                optionValue="slug"
                class="w-full"
              ></Select>
      ...
      <!-- Section Bankrolls - accessible uniquement aux users (pas investor) -->
      <div v-if="canAccessFullProfile" class="card flex flex-col gap-4 w-full">
        <div class="flex justify-between items-center">
  ...
  <Dialog
    v-model:visible="showBankrollDialog"
    :header="editingBankroll ? 'Modifier la bankroll' : 'Ajouter une bankroll'"
    :modal="true"
    ...
    <div class="flex flex-col gap-4">
      <div>
        <label class="block text-sm font-medium mb-2"
          >Nom de la bankroll *</label
        >
        <InputText
          v-model="newBankroll.name"
      ...
      <div>
        ...
        <Textarea
          v-model="newBankroll.comment"
          placeholder="Description ou notes sur cette bankroll..."
          rows="3"
          class="w-full"
        />
      </div>
      ...
      <div>
        <div class="flex justify-between items-center mb-3">
          ...
          <Button
            @click="addBookmakerToBankroll"
            icon="pi pi-plus"
            label="Ajouter un bookmaker"
            class="p-button-sm"
          />
        </div>

        <div
          v-if="newBankroll.bookmakers.length === 0"
        ...
        <div v-else class="space-y-3">
          <div
            v-for="(bookmaker, index) in newBankroll.bookmakers"
            :key="index"
          ...
          >
            <div class="flex-1">
              ...
              <Select
                v-model="bookmaker.bookmaker"
                :options="availableBookmakers"
                optionLabel="name"
                optionValue="slug"
            ...
            <div class="w-32">
              ...
              >
              <InputNumber
                v-model="bookmaker.initialAmount"
                :min="0"
                :maxFractionDigits="2"
                placeholder="0.00"
                class="w-full"
            ...
            <Button
              @click="removeBookmakerFromBankroll(index)"
              icon="pi pi-trash"
  ...
  <!-- Dialog de confirmation de suppression de bankroll -->
  <Dialog
    v-model:visible="showDeleteBankrollDialog"
  ...
  >
    <div class="flex flex-col gap-4">
      ...
      >
        ...
        <div>
          ...
          <p class="text-sm text-red-600 mt-1">
            Cette action supprimera définitivement la bankroll
            <strong>"{{ bankrollToDelete?.bankroll_name }}"</strong>
            ainsi que tous ses paris, événements et bookmakers associés.
          </p>
        </div>
      </div>
      ...
      <div>
        <label class="block text-sm font-medium mb-2">
          Pour confirmer, tapez le nom exact de la bankroll :
        </label>
        <InputText
          v-model="confirmationName"
          :placeholder="bankrollToDelete?.bankroll_name"
          class="w-full"
          autocomplete="off"
        />
      </div>
      ...
      <div class="text-xs text-gray-500">
        ...
        <ul class="list-disc list-inside mt-1 ml-2">
          <li>Tous les paris de cette bankroll</li>
          <li>Tous les événements associés</li>
          <li>Tous les bookmakers liés</li>
          <li>La bankroll elle-même</li>
        </ul>
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <Button
          label="Annuler"
          severity="secondary"
          @click="closeDeleteBankrollDialog"
          :disabled="isDeleting"
        />
        <Button
          label="Supprimer définitivement"
          severity="danger"
          @click="confirmDeleteBankroll"
          :loading="isDeleting"
          :disabled="confirmationName !== bankrollToDelete?.bankroll_name"
        />
      </div>
    </template>
  </Dialog>
