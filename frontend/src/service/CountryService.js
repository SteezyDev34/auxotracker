...
export const CountryService = {
  ...
  async searchCountries(search = "") {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/countries/search?${params}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  async searchCountriesWithPagination(search = "", page = 1, limit = 30) {
    try {
      ...
      console.error(
        "Erreur lors de la recherche des pays avec pagination:",
        error
      );
      throw error;
    }
  ...
  getData() {
    console.warn(
      "CountryService.getData() est déprécié. Utilisez getCountries() à la place."
    );
