...
export const BankrollService = {
  ...
  async getBankroll(bankrollId) {
    try {
      ...
      console.error(
        `Erreur lors de la récupération de la bankroll ${bankrollId}:`,
        error
      );
      throw error;
    }
  ...
  async updateBankroll(bankrollId, bankrollData) {
    try {
      ...
      console.error(
        `Erreur lors de la mise à jour de la bankroll ${bankrollId}:`,
        error
      );
      throw error;
    }
  ...
  async deleteBankroll(bankrollId, bankrollName) {
    try {
      const response = await fetch(`${API_BASE_URL}/bankrolls/${bankrollId}`, {
        method: "DELETE",
        headers: {
      ...
      console.error(
        `Erreur lors de la suppression de la bankroll ${bankrollId}:`,
        error
      );
      throw error;
    }
  ...
  async getBankrollBookmakers(bankrollId) {
    try {
      ...
      return data.user_bookmakers.filter(
        (bookmaker) => bookmaker.users_bankrolls_id === bankrollId
      );
    } catch (error) {
      console.error(
        `Erreur lors de la récupération des bookmakers pour la bankroll ${bankrollId}:`,
        error
      );
      throw error;
    }
  ...
  // Récupérer la liste de tous les bookmakers disponibles
  async getBookmakers() {
    try {
  ...
  // Associer un bookmaker à une bankroll
  async createUserBookmaker(userBookmakerData) {
    try {
      const response = await fetch(`${API_BASE_URL}/user-bookmakers`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify(userBookmakerData),
      });
      ...
      console.error("Erreur lors de l'association du bookmaker:", error);
      throw error;
    }
  },

  // Mettre à jour une association utilisateur-bookmaker
  async updateUserBookmaker(userBookmakerId, userBookmakerData) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/user-bookmakers/${userBookmakerId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  // Supprimer une association utilisateur-bookmaker
  async deleteUserBookmaker(userBookmakerId) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/user-bookmakers/${userBookmakerId}`,
        {
