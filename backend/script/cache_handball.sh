...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/cache_handball_$(date +%Y-%m-%d_%H-%M-%S).log"
echo "=== Cache handball exécuté à $(date) ===" 2>&1 | tee -a "$LOG"
...
mkdir -p "$LOCK_DIR"
LOCK="$LOCK_DIR/cache_handball.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
    echo "$(date) : Script cache_handball déjà en cours (verrou), sortie." 2>&1 | tee -a "$LOG"
    exit 0
fi
trap 'rm -rf "$LOCK"' EXIT

HANDBALL_CACHE_MARKER="$PROJECT_DIR/storage/app/sofascore_cache/handball_CACHE_DONE_$(date +%Y-%m-%d)"
HANDBALL_LEAGUE_PATTERN="$PROJECT_DIR/storage/app/sofascore_cache/handball_LEAGUE_DONE_$(date +%Y-%m-%d)_*"

if [[ "${HANDBALL_FORCE:-}" != "1" ]]; then
    if [[ -f "$HANDBALL_CACHE_MARKER" ]] || compgen -G "$HANDBALL_LEAGUE_PATTERN" >/dev/null; then
        echo "$(date) : ⏭️  Handball Phase 1 déjà faite (marker présent). Skip API." 2>&1 | tee -a "$LOG"
        exit 0
    fi
else
...
if [[ $? -eq 0 ]]; then
    printf "%s" "done" > "$HANDBALL_CACHE_MARKER" 2>/dev/null || true
    echo "$(date) : ✅ Handball Phase 1 (cache) terminée" 2>&1 | tee -a "$LOG"
else
    echo "$(date) : ❌ Erreur lors du cache Handball Phase 1" 2>&1 | tee -a "$LOG"
    exit 1
fi
