...
# Import Phase 2: cache → BDD pour handball
...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/import_handball_$(date +%Y-%m-%d_%H-%M-%S).log"

echo "=== Import handball exécuté à $(date) ===" 2>&1 | tee -a "$LOG"
...
mkdir -p "$LOCK_DIR"
LOCK="$LOCK_DIR/import_handball.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
    echo "$(date) : Script import_handball déjà en cours (verrou), sortie." 2>&1 | tee -a "$LOG"
    exit 0
fi
trap 'rm -rf "$LOCK"' EXIT

# Options d'import (modifiable via variable d'environnement HANDBALL_IMPORT_OPTS)
...
if [[ $? -eq 0 ]]; then
    ...
    echo "$(date) : ❌ Erreur lors de l'import Handball" 2>&1 | tee -a "$LOG"
    exit 1
fi
