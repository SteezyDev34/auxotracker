...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/cache_volleyball_$(date +%Y-%m-%d_%H-%M-%S).log"
echo "=== Cache volleyball exécuté à $(date) ===" 2>&1 | tee -a "$LOG"
...
mkdir -p "$LOCK_DIR"
LOCK="$LOCK_DIR/cache_volleyball.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
    echo "$(date) : Script cache_volleyball déjà en cours (verrou), sortie." 2>&1 | tee -a "$LOG"
    exit 0
fi
trap 'rm -rf "$LOCK"' EXIT

VOLLEYBALL_CACHE_MARKER="$PROJECT_DIR/storage/app/sofascore_cache/volleyball_CACHE_DONE_$(date +%Y-%m-%d)"
VOLLEYBALL_LEAGUE_PATTERN="$PROJECT_DIR/storage/app/sofascore_cache/volleyball_LEAGUE_DONE_$(date +%Y-%m-%d)_*"

if [[ "${VOLLEYBALL_FORCE:-}" != "1" ]]; then
    if [[ -f "$VOLLEYBALL_CACHE_MARKER" ]] || compgen -G "$VOLLEYBALL_LEAGUE_PATTERN" >/dev/null; then
        echo "$(date) : ⏭️  Volleyball Phase 1 déjà faite (marker présent). Skip API." 2>&1 | tee -a "$LOG"
        exit 0
    fi
else
...
if [[ $? -eq 0 ]]; then
    printf "%s" "done" > "$VOLLEYBALL_CACHE_MARKER" 2>/dev/null || true
    echo "$(date) : ✅ Volleyball Phase 1 (cache) terminée" 2>&1 | tee -a "$LOG"
else
    echo "$(date) : ❌ Erreur lors du cache Volleyball Phase 1" 2>&1 | tee -a "$LOG"
    exit 1
fi
