...
# Import Phase 2: cache → BDD pour rugby
...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/import_rugby_$(date +%Y-%m-%d_%H-%M-%S).log"

echo "=== Import rugby exécuté à $(date) ===" 2>&1 | tee -a "$LOG"
...
mkdir -p "$LOCK_DIR"
LOCK="$LOCK_DIR/import_rugby.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
    echo "$(date) : Script import_rugby déjà en cours (verrou), sortie." 2>&1 | tee -a "$LOG"
    exit 0
fi
...
if [[ $? -eq 0 ]]; then
    ...
    echo "$(date) : ❌ Erreur lors de l'import Rugby" 2>&1 | tee -a "$LOG"
    exit 1
fi
