...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/cache_football_$(date +%Y-%m-%d_%H-%M-%S).log"
echo "=== Cache football exécuté à $(date) ===" 2>&1 | tee -a "$LOG"
...
mkdir -p "$LOCK_DIR"
LOCK="$LOCK_DIR/cache_football.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
    echo "$(date) : Script cache_football déjà en cours (verrou), sortie." 2>&1 | tee -a "$LOG"
    exit 0
fi
...
echo "$(date) : Exécution artisan football:import-from-schedule --import-teams --download-logos" 2>&1 | tee -a "$LOG"
