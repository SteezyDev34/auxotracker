...
class ImportBaseballFromCache extends Command
{
    protected $signature = 'baseball:import-from-cache
    ...
    protected $description = 'Phase 2 : Importe les ligues et équipes de baseball depuis les fichiers de cache vers la BDD (aucun appel API). Exécuter baseball:import-from-schedule d\'abord.';
    ...
    public function handle(): int
    {
        ...
        $this->info("⚾ Import Baseball depuis le cache → BDD (Phase 2)");
        $this->line("📅 Date: {$date}");
        ...
        $this->cacheDirectory = storage_path("app/sofascore_cache/baseball_schedule/{$date}");
        $this->line("📂 Cache: {$this->cacheDirectory}");

        if (!is_dir($this->cacheDirectory)) {
            $this->error("❌ Répertoire de cache introuvable: {$this->cacheDirectory}");
            $this->error("   Exécutez d'abord: php artisan baseball:import-from-schedule {$date}");
            return Command::FAILURE;
        }
        $this->line("");

        // Récupérer le sport Baseball
        $this->line("🔍 Recherche du sport Baseball en base...");
        $baseballSport = $this->getBaseballSport();
        if (!$baseballSport) {
            return Command::FAILURE;
        }
        ...
        if (empty($allTournaments)) {
            ...
            $this->warn("   Exécutez d'abord: php artisan baseball:import-from-schedule {$date}");
            $this->displayStats();
        ...
        foreach ($uniqueLeagues as $i => $tournamentData) {
            ...
            $this->processScheduledTournament($tournamentData, $baseballSport, $force, $importTeams, $downloadLogos);
        }
        $this->newLine();
    ...
    private function processScheduledTournament(
        ...
        try {
            ...
            $leagueMarker = storage_path("app/sofascore_cache/baseball_LEAGUE_DONE_{$dateForMarker}_{$sofascoreId}");
            ...
            if (!$country) {
                ...
                Log::warning('Pays introuvable pour le tournoi (baseball from-cache)', [
                    'tournament' => $name,
                    'category' => $category,
                ]);
                return;
            ...
            $league = $this->createOrUpdateLeague($sofascoreId, $name, $slug, $country, $baseballSport, $force);
            ...
            try {
                ...
                Log::warning('Impossible d\'écrire le marker de ligue (baseball)', ['file' => $leagueMarker ?? null, 'error' => $e->getMessage()]);
            }
        } catch (\Exception $e) {
            $this->stats['errors']++;
            ...
            Log::error('Erreur traitement tournoi (baseball from-cache)', [
    ...
    private function findOrCreateCountry(array $categoryData): ?Country
    {
        ...
        try {
            ...
            Log::info('Pays créé automatiquement (baseball from-cache)', [
                'name' => $name,
                'code' => $alpha2,
                'slug' => $slug,
            ]);
            ...
            Log::error('Erreur création pays (baseball from-cache)', [
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
            return null;
    ...
    private function createOrUpdateLeague(
        ...
        try {
            ...
            $this->stats['errors']++;
            Log::error('Erreur création/mise à jour ligue (baseball from-cache)', [
                'sofascore_id' => $sofascoreId,
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadLeagueLogos(League $league, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur téléchargement logo ligue (baseball from-cache)', [
                'league_id' => $league->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function processTeam(array $teamData, League $league, bool $force, bool $downloadLogos): void
    {
        try {
            ...
            if ($duplicateByName) {
                ...
                Log::warning('Doublon potentiel détecté (baseball from-cache)', [
                    'sofascore_id' => $sofascoreId,
                    'name' => $name,
                    'league_id' => $league->id,
            ...
            Log::error('Erreur traitement équipe (baseball from-cache)', [
    ...
    private function downloadTeamLogo(Team $team, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur copie logo équipe (baseball from-cache)', [
                'team_id' => $team->id,
                'sofascore_id' => $team->sofascore_id,
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Récupérer le sport Baseball depuis la base de données.
     */
    private function getBaseballSport(): ?Sport
    {
        $sport = Sport::where('name', 'like', '%baseball%')
            ->orWhere('slug', 'baseball')
            ->first();

        if (!$sport) {
            $this->error("❌ Sport Baseball introuvable en base de données.");
            $this->error("   Assurez-vous qu'un sport avec nom 'Baseball' existe.");
            return null;
        }
    ...
    {
        $this->newLine();
        $this->info('🏁 Import Baseball (from-cache) terminé!');
        $this->newLine();
        ...
        Log::info('Import baseball from-cache terminé', $this->stats);
    }
}
