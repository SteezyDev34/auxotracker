...
class ImportBasketballFromCache extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'basketball:import-from-cache
     ...
     * @var string
     */
    protected $description = 'Phase 2 : Importe les ligues et équipes de basketball depuis les fichiers de cache vers la BDD (aucun appel API). Exécuter basketball:import-from-schedule d\'abord.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🏀 Import Basketball depuis le cache → BDD (Phase 2)");
        $this->line("📅 Date: {$date}");
        ...
        if (!is_dir($this->cacheDirectory)) {
            ...
            $this->error("   Exécutez d'abord: php artisan basketball:import-from-schedule {$date}");
            return Command::FAILURE;
        }
        $this->line("");

        // Récupérer le sport Basketball
        $this->line("🔍 Recherche du sport Basketball en base...");
        $basketballSport = $this->getBasketballSport();
        if (!$basketballSport) {
            return Command::FAILURE;
        }
        ...
        if (empty($allTournaments)) {
            ...
            $this->warn("   Exécutez d'abord: php artisan basketball:import-from-schedule {$date}");
            $this->displayStats();
        ...
        foreach ($uniqueLeagues as $i => $tournamentData) {
            ...
            $this->processScheduledTournament($tournamentData, $basketballSport, $force, $importTeams, $downloadLogos);
        }
        $this->newLine();
    ...
    private function processScheduledTournament(
        ...
        try {
            ...
            $leagueMarker = storage_path("app/sofascore_cache/basketball_LEAGUE_DONE_{$dateForMarker}_{$sofascoreId}");
            ...
            if (!$country) {
                ...
                Log::warning('Pays introuvable pour le tournoi (basketball from-cache)', [
                    'tournament' => $name,
                    'category' => $category,
                ]);
                return;
            ...
            $league = $this->createOrUpdateLeague($sofascoreId, $name, $slug, $country, $basketballSport, $force);
            ...
            try {
                ...
                Log::warning('Impossible d\'écrire le marker de ligue (basketball)', ['file' => $leagueMarker ?? null, 'error' => $e->getMessage()]);
            }
        } catch (\Exception $e) {
            $this->stats['errors']++;
            ...
            Log::error('Erreur traitement tournoi (basketball from-cache)', [
    ...
    private function findOrCreateCountry(array $categoryData): ?Country
    {
        ...
        try {
            ...
            Log::info('Pays créé automatiquement (basketball from-cache)', [
                'name' => $name,
                'code' => $alpha2,
                'slug' => $slug,
            ]);
            ...
            Log::error('Erreur création pays (basketball from-cache)', [
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
            return null;
    ...
    private function createOrUpdateLeague(
        ...
        try {
            ...
            $this->stats['errors']++;
            Log::error('Erreur création/mise à jour ligue (basketball from-cache)', [
                'sofascore_id' => $sofascoreId,
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadLeagueLogos(League $league, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur téléchargement logo ligue (basketball from-cache)', [
                'league_id' => $league->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function processTeam(array $teamData, League $league, bool $force, bool $downloadLogos): void
    {
        try {
            ...
            if ($duplicateByName) {
                ...
                Log::warning('Doublon potentiel détecté (basketball from-cache)', [
                    'sofascore_id' => $sofascoreId,
                    'name' => $name,
                    'league_id' => $league->id,
            ...
            Log::error('Erreur traitement équipe (basketball from-cache)', [
    ...
    private function downloadTeamLogo(Team $team, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur copie logo équipe (basketball from-cache)', [
                'team_id' => $team->id,
                'sofascore_id' => $team->sofascore_id,
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Récupérer le sport Basketball depuis la base de données.
     */
    private function getBasketballSport(): ?Sport
    {
        $sport = Sport::where('sofascore_id', 2)->first();

        if (!$sport) {
            $sport = Sport::where('name', 'like', '%basketball%')
                ->orWhere('slug', 'basketball')
                ->first();
        }

        if (!$sport) {
            $this->error("❌ Sport Basketball introuvable en base de données.");
            $this->error("   Assurez-vous qu'un sport avec sofascore_id=2 ou nom 'Basketball' existe.");
            return null;
        }
    ...
    {
        $this->newLine();
        $this->info('🏁 Import Basketball (from-cache) terminé!');
        $this->newLine();
        ...
        Log::info('Import basketball from-cache terminé', $this->stats);
    }
}
