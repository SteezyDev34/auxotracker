...
class ImportIceHockeyFromSchedule extends Command
{
    protected $signature = 'ice-hockey:import-from-schedule
    ...
    protected $description = 'Phase 1 : Collecte les données de ice-hockey depuis l\'API Sofascore et les stocke en cache local (aucune écriture en BDD). Utiliser ice-hockey:import-from-cache pour la Phase 2.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🏒 Collecte Ice Hockey → cache (Phase 1)");
        $this->line("📅 Date: {$date}");
        ...
        $this->displayStats();
        $this->newLine();
        $this->info("💡 Pour importer en BDD: php artisan ice-hockey:import-from-cache {$date} --import-teams --download-logos");
        return Command::SUCCESS;
    }

    private function setCacheDirectory(string $date): void
    {
        $this->cacheDirectory = storage_path("app/sofascore_cache/ice-hockey_schedule/{$date}");
    ...
    {
        $parentDir = storage_path('app/sofascore_cache/ice-hockey_schedule');
        if (!is_dir($parentDir)) {
            return;
        }
        ...
        if ($cleaned > 0) {
            $this->line("🧹 {$cleaned} répertoire(s) de cache ice-hockey expiré(s) supprimé(s)");
        }
    }
    ...
    {
        $url = "https://www.sofascore.com/api/v1/sport/ice-hockey/scheduled-tournaments/{$date}/page/{$page}";
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::warning('Erreur API scheduled-tournaments ice-hockey', [
                    'url' => $url,
            ...
            Log::error('Exception lors de la récupération des tournois programmés (ice-hockey)', [
                'url' => $url,
                'error' => $e->getMessage(),
    ...
    private function getSeasonId(int $leagueSofascoreId, bool $noCache): ?int
    {
        ...
        try {
            ...
            Log::error('Exception récupération season ID (ice-hockey)', [
    ...
    private function getTeamsFromStandings(int $leagueSofascoreId, int $seasonId, bool $noCache): array
    {
        ...
        try {
            ...
            $this->stats['api_errors']++;
            Log::error('Exception récupération standings (ice-hockey)', [
                'league_sofascore_id' => $leagueSofascoreId,
                'season_id' => $seasonId,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadTeamLogoToCache(int $sofascoreId, string $teamName): string
    {
        ...
        try {
            ...
            Log::warning('Erreur téléchargement logo en cache (ice-hockey Phase 1)', [
                'sofascore_id' => $sofascoreId,
                'team_name' => $teamName,
                'error' => $e->getMessage(),
            ]);
    ...
    private function handleForbiddenError($response, string $url): void
    {
        ...
        Log::error('Erreur 403 - Challenge détecté (ice-hockey)', [
            'url' => $url,
            'reason' => $reason,
            'response_body' => $body,
        ]);
    ...
    private function displayStats(): void
    {
        ...
        Log::info('Ice-hockey Phase 1 (cache) terminée', $this->stats);
    }
}
