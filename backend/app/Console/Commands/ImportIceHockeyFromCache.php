...
class ImportIceHockeyFromCache extends Command
{
    protected $signature = 'ice-hockey:import-from-cache
    ...
    protected $description = 'Phase 2 : Importe les ligues et équipes de ice-hockey depuis les fichiers de cache vers la BDD (aucun appel API). Exécuter ice-hockey:import-from-schedule d\'abord.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🏒 Import Ice Hockey depuis le cache → BDD (Phase 2)");
        $this->line("📅 Date: {$date}");
        ...
        $this->cacheDirectory = storage_path("app/sofascore_cache/ice_hockey_schedule/{$date}");
        $this->line("📂 Cache: {$this->cacheDirectory}");

        if (!is_dir($this->cacheDirectory)) {
            $this->error("❌ Répertoire de cache introuvable: {$this->cacheDirectory}");
            $this->error("   Exécutez d'abord: php artisan ice-hockey:import-from-schedule {$date}");
            return Command::FAILURE;
        }
        $this->line("");

        $this->line("🔍 Recherche du sport Ice Hockey en base...");
        $sport = $this->getIceHockeySport();
        if (!$sport) {
            return Command::FAILURE;
        }
        ...
        if (empty($allTournaments)) {
            ...
            $this->warn("   Exécutez d'abord: php artisan ice-hockey:import-from-schedule {$date}");
            $this->displayStats();
    ...
    private function processScheduledTournament(array $tournamentData, Sport $sport, bool $force, bool $importTeams, bool $downloadLogos): void
    {
        ...
        try {
            ...
            $leagueMarker = storage_path("app/sofascore_cache/ice_hockey_LEAGUE_DONE_{$dateForMarker}_{$sofascoreId}");
            ...
            if (!$country) {
                ...
                Log::warning('Pays introuvable pour le tournoi (ice_hockey from-cache)', [
                    'tournament' => $name,
                    'category' => $category,
                ]);
                return;
            ...
            try {
                ...
                Log::warning('Impossible d\'écrire le marker de ligue (ice_hockey)', ['file' => $leagueMarker ?? null, 'error' => $e->getMessage()]);
            }
        } catch (\Exception $e) {
            $this->stats['errors']++;
            ...
            Log::error('Erreur traitement tournoi (ice_hockey from-cache)', [
    ...
    private function findOrCreateCountry(array $categoryData): ?Country
    {
        ...
        try {
            ...
            Log::info('Pays créé automatiquement (ice_hockey from-cache)', [
                'name' => $name,
                'code' => $alpha2,
                'slug' => $slug,
            ]);
            ...
            Log::error('Erreur création pays (ice_hockey from-cache)', [
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
            return null;
    ...
    private function createOrUpdateLeague(int $sofascoreId, string $name, ?string $slug, Country $country, Sport $sport, bool $force): ?League
    {
        try {
            ...
            $this->stats['errors']++;
            Log::error('Erreur création/mise à jour ligue (ice_hockey from-cache)', [
                'sofascore_id' => $sofascoreId,
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadLeagueLogos(League $league, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur téléchargement logo ligue (ice_hockey from-cache)', [
                'league_id' => $league->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function processTeam(array $teamData, League $league, bool $force, bool $downloadLogos): void
    {
        try {
            ...
            if ($duplicateByName) {
                ...
                Log::warning('Doublon potentiel détecté (ice_hockey from-cache)', [
                    'sofascore_id' => $sofascoreId,
                    'name' => $name,
                    'league_id' => $league->id,
            ...
            Log::error('Erreur traitement équipe (ice_hockey from-cache)', [
        ...
        try {
            ...
            Log::warning('Erreur copie logo équipe (ice_hockey from-cache)', [
                'team_id' => $team->id,
                'sofascore_id' => $team->sofascore_id,
                'error' => $e->getMessage(),
            ]);
    ...
    private function getIceHockeySport(): ?Sport
    {
        $sport = Sport::where('name', 'like', '%ice hockey%')
            ->orWhere('slug', 'ice-hockey')
            ->orWhere('slug', 'ice_hockey')
            ->first();

        if (!$sport) {
            // fallback to generic hockey name if exact not found
            $sport = Sport::where('name', 'like', '%hockey%')->first();
        }

        if (!$sport) {
            $this->error("❌ Sport Ice Hockey introuvable en base de données.");
            $this->error("   Assurez-vous qu'un sport 'Ice Hockey' ou 'Hockey' existe.");
            return null;
        }
    ...
    {
        $this->newLine();
        $this->info('🏁 Import Ice Hockey (from-cache) terminé!');
        $this->newLine();
        ...
        Log::info('Import ice_hockey from-cache terminé', $this->stats);
    }
}
