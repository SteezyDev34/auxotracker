...
class ImportFootballFromSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'football:import-from-schedule
     ...
     * @var string
     */
    protected $description = 'Phase 1 : Collecte les données de football depuis l\'API Sofascore et les stocke en cache local (aucune écriture en BDD). Utiliser football:import-from-cache pour la Phase 2.';
    ...
    public function handle(): int
    {
        ...
        $this->info("⚽ Collecte Football → cache (Phase 1)");
        $this->line("📅 Date: {$date}");
        ...
        $this->displayStats();
        $this->newLine();
        $this->info("💡 Pour importer en BDD: php artisan football:import-from-cache {$date} --import-teams --download-logos");
        return Command::SUCCESS;
    }
    ...
    {
        $parentDir = storage_path('app/sofascore_cache/football_schedule');
        if (!is_dir($parentDir)) {
            return;
        }
        ...
        if ($cleaned > 0) {
            $this->line("🧹 {$cleaned} répertoire(s) de cache football expiré(s) supprimé(s)");
        }
    }
    ...
    {
        $url = "https://www.sofascore.com/api/v1/sport/football/scheduled-tournaments/{$date}/page/{$page}";
    ...
    private function downloadTeamLogoToCache(int $sofascoreId, string $teamName): string
    {
        ...
        try {
            ...
            Log::warning('Erreur téléchargement logo en cache (football Phase 1)', [
                'sofascore_id' => $sofascoreId,
                'team_name' => $teamName,
                'error' => $e->getMessage(),
            ]);
    ...
    private function displayStats(): void
    {
        ...
        Log::info('Football Phase 1 (cache) terminée', $this->stats);
    }
}
