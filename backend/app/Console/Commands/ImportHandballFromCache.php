...
class ImportHandballFromCache extends Command
{
    protected $signature = 'handball:import-from-cache
    ...
    protected $description = 'Phase 2 : Importe les ligues et équipes de handball depuis les fichiers de cache vers la BDD (aucun appel API). Exécuter handball:import-from-schedule d\'abord.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🤾 Import Handball depuis le cache → BDD (Phase 2)");
        $this->line("📅 Date: {$date}");
        ...
        if (!is_dir($this->cacheDirectory)) {
            ...
            $this->error("   Exécutez d'abord: php artisan handball:import-from-schedule {$date}");
            return Command::FAILURE;
        }
        $this->line("");

        $this->line("🔍 Recherche du sport Handball en base...");
        $sport = $this->getHandballSport();
        if (!$sport) {
            return Command::FAILURE;
        }
        ...
        if (empty($allTournaments)) {
            ...
            $this->warn("   Exécutez d'abord: php artisan handball:import-from-schedule {$date}");
            $this->displayStats();
    ...
    private function processScheduledTournament(array $tournamentData, Sport $sport, bool $force, bool $importTeams, bool $downloadLogos): void
    {
        ...
        try {
            ...
            $leagueMarker = storage_path("app/sofascore_cache/handball_LEAGUE_DONE_{$dateForMarker}_{$sofascoreId}");
            ...
            if (!$country) {
                ...
                Log::warning('Pays introuvable pour le tournoi (handball from-cache)', [
                    'tournament' => $name,
                    'category' => $category,
                ]);
                return;
            ...
            try {
                ...
                Log::warning('Impossible d\'écrire le marker de ligue (handball)', ['file' => $leagueMarker ?? null, 'error' => $e->getMessage()]);
            }
        } catch (\Exception $e) {
            $this->stats['errors']++;
            ...
            Log::error('Erreur traitement tournoi (handball from-cache)', [
    ...
    private function findOrCreateCountry(array $categoryData): ?Country
    {
        ...
        try {
            ...
            Log::info('Pays créé automatiquement (handball from-cache)', [
                'name' => $name,
                'code' => $alpha2,
                'slug' => $slug,
            ]);
            ...
            Log::error('Erreur création pays (handball from-cache)', [
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
            return null;
    ...
    private function createOrUpdateLeague(int $sofascoreId, string $name, ?string $slug, Country $country, Sport $sport, bool $force): ?League
    {
        try {
            ...
            $this->stats['errors']++;
            Log::error('Erreur création/mise à jour ligue (handball from-cache)', [
                'sofascore_id' => $sofascoreId,
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadLeagueLogos(League $league, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur téléchargement logo ligue (handball from-cache)', [
                'league_id' => $league->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function processTeam(array $teamData, League $league, bool $force, bool $downloadLogos): void
    {
        try {
            ...
            if ($duplicateByName) {
                ...
                Log::warning('Doublon potentiel détecté (handball from-cache)', [
                    'sofascore_id' => $sofascoreId,
                    'name' => $name,
                    'league_id' => $league->id,
            ...
            // Créer ou mettre à jour l'équipe (sans écrire directement nickname)
            $attributes = [
                'name' => $name,
                'slug' => $slug,
                'sofascore_id' => $sofascoreId,
            ...
            Log::error('Erreur traitement équipe (handball from-cache)', [
    ...
    private function downloadTeamLogo(Team $team, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur copie logo équipe (handball from-cache)', [
                'team_id' => $team->id,
                'sofascore_id' => $team->sofascore_id,
                'error' => $e->getMessage(),
            ]);
    ...
    private function getHandballSport(): ?Sport
    {
        $sport = Sport::where('name', 'like', '%handball%')
            ->orWhere('slug', 'handball')
            ->first();

        if (!$sport) {
            $this->error("❌ Sport Handball introuvable en base de données.");
            $this->error("   Assurez-vous qu'un sport avec nom 'Handball' existe.");
            return null;
        }
    ...
    {
        $this->newLine();
        $this->info('🏁 Import Handball (from-cache) terminé!');
        $this->newLine();
        ...
        Log::info('Import handball from-cache terminé', $this->stats);
    }
}
