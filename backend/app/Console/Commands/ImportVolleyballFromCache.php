...
class ImportVolleyballFromCache extends Command
{
    protected $signature = 'volleyball:import-from-cache
    ...
    protected $description = 'Phase 2 : Importe les ligues et équipes de volleyball depuis les fichiers de cache vers la BDD (aucun appel API). Exécuter volleyball:import-from-schedule d\'abord.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🏐 Import Volleyball depuis le cache → BDD (Phase 2)");
        $this->line("📅 Date: {$date}");
        ...
        if (!is_dir($this->cacheDirectory)) {
            ...
            $this->error("   Exécutez d'abord: php artisan volleyball:import-from-schedule {$date}");
            return Command::FAILURE;
        }
        $this->line("");

        $this->line("🔍 Recherche du sport Volleyball en base...");
        $sport = $this->getVolleyballSport();
        if (!$sport) {
            return Command::FAILURE;
        }
        ...
        if (empty($allTournaments)) {
            ...
            $this->warn("   Exécutez d'abord: php artisan volleyball:import-from-schedule {$date}");
            $this->displayStats();
    ...
    private function processScheduledTournament(array $tournamentData, Sport $sport, bool $force, bool $importTeams, bool $downloadLogos): void
    {
        ...
        try {
            ...
            $leagueMarker = storage_path("app/sofascore_cache/volleyball_LEAGUE_DONE_{$dateForMarker}_{$sofascoreId}");
            ...
            if (!$country) {
                ...
                Log::warning('Pays introuvable pour le tournoi (volleyball from-cache)', [
                    'tournament' => $name,
                    'category' => $category,
                ]);
                return;
            ...
            try {
                ...
                Log::warning('Impossible d\'écrire le marker de ligue (volleyball)', ['file' => $leagueMarker ?? null, 'error' => $e->getMessage()]);
            }
        } catch (\Exception $e) {
            $this->stats['errors']++;
            ...
            Log::error('Erreur traitement tournoi (volleyball from-cache)', [
    ...
    private function findOrCreateCountry(array $categoryData): ?Country
    {
        ...
        try {
            ...
            Log::info('Pays créé automatiquement (volleyball from-cache)', [
                'name' => $name,
                'code' => $alpha2,
                'slug' => $slug,
            ]);
            ...
            Log::error('Erreur création pays (volleyball from-cache)', [
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
            return null;
    ...
    private function createOrUpdateLeague(int $sofascoreId, string $name, ?string $slug, Country $country, Sport $sport, bool $force): ?League
    {
        try {
            ...
            $this->stats['errors']++;
            Log::error('Erreur création/mise à jour ligue (volleyball from-cache)', [
                'sofascore_id' => $sofascoreId,
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadLeagueLogos(League $league, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur téléchargement logo ligue (volleyball from-cache)', [
                'league_id' => $league->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function processTeam(array $teamData, League $league, bool $force, bool $downloadLogos): void
    {
        try {
            ...
            if ($duplicateByName) {
                ...
                Log::warning('Doublon potentiel détecté (volleyball from-cache)', [
                    'sofascore_id' => $sofascoreId,
                    'name' => $name,
                    'league_id' => $league->id,
            ...
            // Créer ou mettre à jour l'équipe (sans écrire nickname directement)
            $attributes = [
                'name' => $name,
                'slug' => $slug,
                'sofascore_id' => $sofascoreId,
            ...
            Log::error('Erreur traitement équipe (volleyball from-cache)', [
    ...
    private function downloadTeamLogo(Team $team, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur copie logo équipe (volleyball from-cache)', [
                'team_id' => $team->id,
                'sofascore_id' => $team->sofascore_id,
                'error' => $e->getMessage(),
            ]);
    ...
    private function getVolleyballSport(): ?Sport
    {
        $sport = Sport::where('name', 'like', '%volley%')
            ->orWhere('slug', 'volleyball')
            ->first();

        if (!$sport) {
            $this->error("❌ Sport Volleyball introuvable en base de données.");
            $this->error("   Assurez-vous qu'un sport avec nom 'Volleyball' existe.");
            return null;
        }
    ...
    {
        $this->newLine();
        $this->info('🏁 Import Volleyball (from-cache) terminé!');
        $this->newLine();
        ...
        Log::info('Import volleyball from-cache terminé', $this->stats);
    }
}
