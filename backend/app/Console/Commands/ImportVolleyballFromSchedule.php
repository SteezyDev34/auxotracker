...
class ImportVolleyballFromSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'volleyball:import-from-schedule
     ...
     * @var string
     */
    protected $description = 'Phase 1 : Collecte les données de volleyball depuis l\'API Sofascore et les stocke en cache local (aucune écriture en BDD). Utiliser volleyball:import-from-cache pour la Phase 2.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🏐 Collecte Volleyball → cache (Phase 1)");
        $this->line("📅 Date: {$date}");
        ...
        $this->displayStats();
        $this->newLine();
        $this->info("💡 Pour importer en BDD: php artisan volleyball:import-from-cache {$date} --import-teams --download-logos");
        return Command::SUCCESS;
    }
    ...
    {
        $this->cacheDirectory = storage_path("app/sofascore_cache/volleyball_schedule/{$date}");
    ...
    {
        $parentDir = storage_path('app/sofascore_cache/volleyball_schedule');
        if (!is_dir($parentDir)) {
            return;
        }
        ...
        if ($cleaned > 0) {
            $this->line("🧹 {$cleaned} répertoire(s) de cache volleyball expiré(s) supprimé(s)");
        }
    }
    ...
    {
        $url = "https://www.sofascore.com/api/v1/sport/volleyball/scheduled-tournaments/{$date}/page/{$page}";
    ...
    private function downloadTeamLogoToCache(int $sofascoreId, string $teamName): string
    {
        ...
        try {
            ...
            Log::warning('Erreur téléchargement logo en cache (volleyball Phase 1)', [
                'sofascore_id' => $sofascoreId,
                'team_name' => $teamName,
                'error' => $e->getMessage(),
            ]);
    ...
    private function displayStats(): void
    {
        ...
        Log::info('Volleyball Phase 1 (cache) terminée', $this->stats);
    }
}
