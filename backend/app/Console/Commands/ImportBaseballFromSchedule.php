...
class ImportBaseballFromSchedule extends Command
{
    protected $signature = 'baseball:import-from-schedule
    ...
    protected $description = 'Phase 1 : Collecte les données de baseball depuis l\'API Sofascore et les stocke en cache local (aucune écriture en BDD). Utiliser baseball:import-from-cache pour la Phase 2.';
    ...
    public function handle(): int
    {
        ...
        $this->info("⚾ Collecte Baseball → cache (Phase 1)");
        $this->line("📅 Date: {$date}");
        ...
        $this->displayStats();
        $this->newLine();
        $this->info("💡 Pour importer en BDD: php artisan baseball:import-from-cache {$date} --import-teams --download-logos");
        return Command::SUCCESS;
    }
    ...
    {
        $parentDir = storage_path('app/sofascore_cache/baseball_schedule');
        if (!is_dir($parentDir)) {
            return;
        }
        ...
        if ($cleaned > 0) {
            $this->line("🧹 {$cleaned} répertoire(s) de cache baseball expiré(s) supprimé(s)");
        }
    }
    ...
    {
        $url = "https://www.sofascore.com/api/v1/sport/baseball/scheduled-tournaments/{$date}/page/{$page}";
    ...
    private function downloadTeamLogoToCache(int $sofascoreId, string $teamName): string
    {
        ...
        try {
            ...
            Log::warning('Erreur téléchargement logo en cache (baseball Phase 1)', [
                'sofascore_id' => $sofascoreId,
                'team_name' => $teamName,
                'error' => $e->getMessage(),
            ]);
    ...
    private function displayStats(): void
    {
        ...
        Log::info('Baseball Phase 1 (cache) terminée', $this->stats);
    }
}
