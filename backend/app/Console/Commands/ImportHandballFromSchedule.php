...
class ImportHandballFromSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'handball:import-from-schedule
     ...
     * @var string
     */
    protected $description = 'Phase 1 : Collecte les données de handball depuis l\'API Sofascore et les stocke en cache local (aucune écriture en BDD). Utiliser handball:import-from-cache pour la Phase 2.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🤾 Collecte Handball → cache (Phase 1)");
        $this->line("📅 Date: {$date}");
        ...
        $this->displayStats();
        $this->newLine();
        $this->info("💡 Pour importer en BDD: php artisan handball:import-from-cache {$date} --import-teams --download-logos");
        return Command::SUCCESS;
    }
    ...
    {
        $this->cacheDirectory = storage_path("app/sofascore_cache/handball_schedule/{$date}");
    ...
    {
        $parentDir = storage_path('app/sofascore_cache/handball_schedule');
        if (!is_dir($parentDir)) {
            return;
        }
        ...
        if ($cleaned > 0) {
            $this->line("🧹 {$cleaned} répertoire(s) de cache handball expiré(s) supprimé(s)");
        }
    }
    ...
    {
        $url = "https://www.sofascore.com/api/v1/sport/handball/scheduled-tournaments/{$date}/page/{$page}";
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::warning('Erreur API scheduled-tournaments (handball)', [
                    'url' => $url,
            ...
            Log::error('Exception lors de la récupération des tournois programmés (handball)', [
                'url' => $url,
                'error' => $e->getMessage(),
    ...
    private function getSeasonId(int $leagueSofascoreId, bool $noCache): ?int
    {
        ...
        try {
            ...
            Log::error('Exception récupération season ID (handball)', [
    ...
    private function getTeamsFromStandings(int $leagueSofascoreId, int $seasonId, bool $noCache): array
    {
        ...
        try {
            ...
            $this->stats['api_errors']++;
            Log::error('Exception récupération standings (handball)', [
                'league_sofascore_id' => $leagueSofascoreId,
                'season_id' => $seasonId,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadTeamLogoToCache(int $sofascoreId, string $teamName): string
    {
        ...
        try {
            ...
            Log::warning('Erreur téléchargement logo en cache (handball Phase 1)', [
                'sofascore_id' => $sofascoreId,
                'team_name' => $teamName,
                'error' => $e->getMessage(),
            ]);
