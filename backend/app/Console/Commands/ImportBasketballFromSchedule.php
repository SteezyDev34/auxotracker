...
class ImportBasketballFromSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'basketball:import-from-schedule
     ...
     * @var string
     */
    protected $description = 'Phase 1 : Collecte les données de basketball depuis l\'API Sofascore et les stocke en cache local (aucune écriture en BDD). Utiliser basketball:import-from-cache pour la Phase 2.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🏀 Collecte Basketball → cache (Phase 1)");
        $this->line("📅 Date: {$date}");
        ...
        // Nettoyage automatique des anciens caches (> 7 jours)
        if (!$noCache) {
            $this->cleanExpiredCache();
        }
        $this->line("");

        // Étape 1 : Récupérer les tournois programmés (avec pagination)
        ...
        // Étape 2 : Dédupliquer par uniqueTournament.id
        ...
        // Étape 3 : Si --import-teams, pré-charger seasons + standings dans le cache
        if ($importTeams) {
        ...
        $this->displayStats();
        $this->newLine();
        $this->info("💡 Pour importer en BDD: php artisan basketball:import-from-cache {$date} --import-teams --download-logos");
        return Command::SUCCESS;
    }
    ...
    /**
     * Initialiser le répertoire de cache pour cette date.
     */
    private function setCacheDirectory(string $date): void
    {
        $this->cacheDirectory = storage_path("app/sofascore_cache/basketball_schedule/{$date}");
    ...
    /**
     * Nettoyer les répertoires de cache datés de plus de 7 jours.
     * (Harmonisé avec Tennis : nettoyage automatique)
     */
    private function cleanExpiredCache(): void
    {
        $parentDir = storage_path('app/sofascore_cache/basketball_schedule');
        if (!is_dir($parentDir)) {
            return;
        }
        ...
        if ($cleaned > 0) {
            $this->line("🧹 {$cleaned} répertoire(s) de cache basketball expiré(s) supprimé(s)");
        }
    }

    /**
     * Lire un fichier de cache JSON.
     * Retourne null si fichier absent. Retourne le tableau si valide.
    ...
    private function readCache(string $cacheFile): ?array
    {
        ...
        if (!empty($data['_negative_cache'])) {
            ...
            if ($cacheAge < 86400) { // 24h
                return $data; // Sera détecté par l'appelant via '_negative_cache'
            }
            // Tombstone expiré : on le supprime et on retourne null pour re-tenter
            @unlink($cacheFile);
            return null;
        }
    ...
    /**
     * Écrire un fichier de cache JSON.
     */
     ...
     * Expire automatiquement après 24h via readCache().
     */
    ...
    // ─── FETCH SCHEDULED TOURNAMENTS (PAGINATED) ───────────────────────

    /**
     * Récupérer toutes les pages de tournois programmés.
     */
    ...
    /**
     * Récupérer une page de tournois programmés.
     */
    private function fetchScheduledTournamentsPage(string $date, int $page, bool $noCache): ?array
    {
        $url = "https://www.sofascore.com/api/v1/sport/basketball/scheduled-tournaments/{$date}/page/{$page}";
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::warning('Erreur API scheduled-tournaments basketball', [
                    'url' => $url,
                    'status' => $response->status(),
                    'body' => $body,
                    'response_headers' => $response->headers(),
                ]);
                // Cache négatif pour éviter de re-tenter cette page
                if (!$noCache) {
            ...
            Log::error('Exception lors de la récupération des tournois programmés basketball', [
                'url' => $url,
                'error' => $e->getMessage(),
    ...
    /**
     * Charger tous les tournois depuis les fichiers de cache.
     */
    // ─── DEDUPLICATION ──────────────────────────────────────────────────
    ...
    /**
     * Compter les uniqueTournament distincts.
     */
    ...
    private function getSeasonId(int $leagueSofascoreId, bool $noCache): ?int
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                // Cache négatif pour éviter de re-tenter cette ligue
            ...
            Log::error('Exception récupération season ID basketball', [
    ...
    /**
     * Extraire l'ID de saison depuis les données featured events.
     */
    ...
    /**
     * Récupérer les équipes depuis les standings d'une saison.
     */
    private function getTeamsFromStandings(int $leagueSofascoreId, int $seasonId, bool $noCache): array
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                $this->error("      📄 " . substr($response->body(), 0, 300));
                // Cache négatif pour éviter de re-tenter ces standings
                if (!$noCache && $response->status() !== 403) {
                    $this->writeNegativeCache($cacheFile, $url, $response->status());
            ...
            $this->stats['api_errors']++;
            Log::error('Exception récupération standings basketball', [
                'league_sofascore_id' => $leagueSofascoreId,
                'season_id' => $seasonId,
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Extraire les équipes du JSON standings.
     */
    ...
    /**
     * Télécharger le logo d'une équipe depuis l'API Sofascore et le stocker dans le cache.
     * Fichier cible : {cacheDirectory}/team_logos/{sofascoreId}.png
     *
     * @return string 'downloaded', 'cached', 'skipped' ou 'error'
     */
    private function downloadTeamLogoToCache(int $sofascoreId, string $teamName): string
    {
        ...
        // Si le logo est déjà en cache, on skip
        if (file_exists($logoPath) && filesize($logoPath) > 0) {
            return 'cached';
        }

        // Vérifier le cache négatif (éviter les retentatives)
        ...
        try {
            ...
            // Cache négatif pour erreurs 403/404
            ...
            Log::warning('Erreur téléchargement logo en cache (basketball Phase 1)', [
                'sofascore_id' => $sofascoreId,
                'team_name' => $teamName,
                'error' => $e->getMessage(),
            ]);
    ...
    private function displayStats(): void
    {
        ...
        Log::info('Basketball Phase 1 (cache) terminée', $this->stats);
    }
}
