...
class ImportFutsalFromCache extends Command
{
    protected $signature = 'futsal:import-from-cache
    ...
    protected $description = 'Phase 2 : Importe les ligues et équipes de futsal depuis les fichiers de cache vers la BDD (aucun appel API). Exécuter futsal:import-from-schedule d\'abord.';
    ...
    public function handle(): int
    {
        ...
        $this->info("🥅 Import Futsal depuis le cache → BDD (Phase 2)");
        $this->line("📅 Date: {$date}");
        ...
        if (!is_dir($this->cacheDirectory)) {
            ...
            $this->error("   Exécutez d'abord: php artisan futsal:import-from-schedule {$date}");
            return Command::FAILURE;
        }
        $this->line("");

        // Récupérer le sport Futsal
        $this->line("🔍 Recherche du sport Futsal en base...");
        $futsalSport = $this->getFutsalSport();
        if (!$futsalSport) {
            return Command::FAILURE;
        }
        ...
        if (empty($allTournaments)) {
            ...
            $this->warn("   Exécutez d'abord: php artisan futsal:import-from-schedule {$date}");
            $this->displayStats();
        ...
        foreach ($uniqueLeagues as $i => $tournamentData) {
            ...
            $this->processScheduledTournament($tournamentData, $futsalSport, $force, $importTeams, $downloadLogos);
        }
        $this->newLine();
    ...
    // (les méthodes readCache, loadTournamentsFromCache, deduplicateTournaments, processScheduledTournament,
    // findOrCreateCountry, createOrUpdateLeague, downloadLeagueLogos, importTeamsFromCache, getSeasonIdFromCache,
    // getTeamsFromStandingsCache, teamPlayersCacheExists, processTeam, downloadTeamLogo, getFutsalSport, displayStats)
    // sont identiques à la version football et ont été copiées pour assurer un comportement uniforme.
    ...
    private function processScheduledTournament(array $tournamentData, Sport $sport, bool $force, bool $importTeams, bool $downloadLogos): void
    {
        ...
        try {
            ...
            $leagueMarker = storage_path("app/sofascore_cache/futsal_LEAGUE_DONE_{$dateForMarker}_{$sofascoreId}");
            ...
            if (!$country) {
                ...
                Log::warning('Pays introuvable pour le tournoi (futsal from-cache)', [
                    'tournament' => $name,
                    'category' => $category,
                ]);
                return;
            ...
            try {
                ...
                Log::warning('Impossible d\'écrire le marker de ligue (futsal)', ['file' => $leagueMarker ?? null, 'error' => $e->getMessage()]);
            }
        } catch (\Exception $e) {
            $this->stats['errors']++;
            ...
            Log::error('Erreur traitement tournoi (futsal from-cache)', [
    ...
    private function findOrCreateCountry(array $categoryData): ?Country
    {
        ...
        try {
            ...
            Log::info('Pays créé automatiquement (futsal from-cache)', [
                'name' => $name,
                'code' => $alpha2,
                'slug' => $slug,
            ]);
            ...
            Log::error('Erreur création pays (futsal from-cache)', [
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
            return null;
    ...
    private function createOrUpdateLeague(int $sofascoreId, string $name, ?string $slug, Country $country, Sport $sport, bool $force): ?League
    {
        try {
            ...
            $this->stats['errors']++;
            Log::error('Erreur création/mise à jour ligue (futsal from-cache)', [
                'sofascore_id' => $sofascoreId,
                'name' => $name,
                'error' => $e->getMessage(),
            ]);
    ...
    private function downloadLeagueLogos(League $league, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur téléchargement logo ligue (futsal from-cache)', [
                'league_id' => $league->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function processTeam(array $teamData, League $league, bool $force, bool $downloadLogos): void
    {
        try {
            ...
            if ($duplicateByName) {
                ...
                Log::warning('Doublon potentiel détecté (futsal from-cache)', [
                    'sofascore_id' => $sofascoreId,
                    'name' => $name,
                    'league_id' => $league->id,
            ...
            Log::error('Erreur traitement équipe (futsal from-cache)', [
    ...
    private function downloadTeamLogo(Team $team, bool $force): void
    {
        try {
            ...
            Log::warning('Erreur copie logo équipe (futsal from-cache)', [
                'team_id' => $team->id,
                'sofascore_id' => $team->sofascore_id,
                'error' => $e->getMessage(),
            ]);
    ...
    private function getFutsalSport(): ?Sport
    {
        $sport = Sport::where('name', 'like', '%futsal%')
            ->orWhere('slug', 'futsal')
            ->first();

        if (!$sport) {
            $this->error("❌ Sport Futsal introuvable en base de données.");
            $this->error("   Assurez-vous qu'un sport avec nom 'Futsal' existe.");
            return null;
        }
    ...
    {
        $this->newLine();
        $this->info('🏁 Import Futsal (from-cache) terminé!');
        $this->newLine();
        ...
        Log::info('Import futsal from-cache terminé', $this->stats);
    }
}
