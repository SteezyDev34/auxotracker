# `basketball:import-from-cache` — Importer le basketball depuis le cache (Phase 2)

## Informations générales

| Propriété | Valeur |
|---|---|
| **Commande** | `php artisan basketball:import-from-cache` |
| **Classe** | `App\Console\Commands\ImportBasketballFromCache` |
| **Fichier** | `app/Console/Commands/ImportBasketballFromCache.php` |
| **Catégorie** | Import / Basketball |
| **Phase** | **Phase 2** — Cache → BDD (aucun appel API) |

## Description

Importe les ligues et équipes de basketball depuis les fichiers de cache local vers la base de données. C'est la **Phase 2** du processus d'import en 2 phases. La Phase 1 (`basketball:import-from-schedule`) collecte les données depuis l'API Sofascore et les stocke en cache ; cette commande lit le cache et persiste les données en BDD.
...
Phase 2 : basketball:import-from-cache     →  fichiers cache (JSON) → BDD (Country, League, Team)
```
...
```bash
php artisan basketball:import-from-cache [date] [--force] [--import-teams] [--download-logos]
```
...
1. **Chargement du cache** : lit les fichiers `page_{n}.json` depuis `storage/app/sofascore_cache/basketball_schedule/{date}/`
2. **Récupération du sport** : cherche le sport Basketball en base (sofascore_id=2)
...
```
storage/app/sofascore_cache/basketball_schedule/{date}/
...
php artisan basketball:import-from-cache 2025-06-01 --force --import-teams --download-logos
```
...
- [`basketball:import-from-schedule`](basketball-import-from-schedule.md) — Phase 1 (API → cache)
- [`football:import-from-cache`](football-import-from-cache.md) — Équivalent football
